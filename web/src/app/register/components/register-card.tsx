          ...
          <div className="space-y-2">
            ...
            <Select value={config.mode || "total"} onValueChange={(value) => setMode(value as "total" | "quota" | "available")} disabled={config.enabled}>
              ...
              <SelectContent>
                ...
                <SelectItem value="quota">池子剩余额度</SelectItem>
                    ...
                    {type === "ddg_mail" ? (
                      <>
                        ...
                        <div className="space-y-2">
                          ...
                          <Input value={String(provider.cf_inbox_jwt || "")} onChange={(event) => updateProvider(index, { cf_inbox_jwt: event.target.value })} className="h-10 rounded-xl border-stone-200 bg-white" disabled={config.enabled} placeholder="Cloudflare 收件箱固定收件地址 JWT" />
                        </div>
                        <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
                          <p className="mb-1 font-medium">使用说明</p>
                          <ol className="list-decimal list-inside space-y-0.5">
                            <li>先在 <a href="https://duckduckgo.com/email/" target="_blank" rel="noreferrer" className="underline">DuckDuckGo Email Protection</a> 登录并把转发目标设为 CF 收件箱地址。</li>
                            <li>DDG Token 可从浏览器 DevTools 的 `quack.duckduckgo.com` 请求头 `Authorization: Bearer ...` 获取。</li>
                            <li>CF Inbox JWT 从 Cloudflare 临时邮箱后端固定收件箱中获取。</li>
                            <li>所有 `@duck.com` 别名来信会转发到同一个 CF 收件箱，系统按 `To:` 头自动匹配。</li>
                          </ol>
                        </div>
                      </>
                    ) : null}
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-4 gap-2">
            {[
              ...
              ["平均单个耗时", `${stats.avg_seconds || 0}s`],
              ["当前额度", stats.current_quota || 0],
              ["正常账号", stats.current_available || 0],
            ].map(([label, value]) => (
              <div key={String(label)} className="border border-stone-200 bg-white/70 px-3 py-2">
