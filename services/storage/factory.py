...
def create_storage_backend(data_dir: Path) -> StorageBackend:
    ...
    if backend_type in ("sqlite", "postgres", "postgresql", "mysql", "database"):
        ...
        if _is_postgres_url(database_url):
            try:
                ...
                raise RuntimeError(
                    "PostgreSQL storage dependencies are not installed. "
                    "Rebuild the image with ENABLE_POSTGRES_STORAGE=1."
                ) from exc
    ...
    if backend_type == "git":
        ...
        try:
            ...
            raise RuntimeError(
                "Git storage dependencies are not installed. "
                "Rebuild the image with ENABLE_GIT_STORAGE=1."
            ) from exc
